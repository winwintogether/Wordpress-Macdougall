...
( function( $, _ ) {
    ...
    eml.changeSorting = {
        ...
        changeOrderby: function( event ) {
            ...
            reverse.model.set( 'disabled', 'rand' === $( event.target ).val() );


            library.props.set( 'orderby', orderby );

            if ( undefined === order || 'menuOrder' === orderby ) {
    ...
    _.extend( media.view.Settings.Gallery.prototype, {
        ...
        render: function() {
            ...
            $el.find( 'input[data-setting=_orderbyRandom]' ).parent( 'label' ).replaceWith( media.template( 'eml-pro-gallery-order' )(this.options) );

            if ( $el.find( '.setting.eml-filter-based' ).length ) {
                $el.find( '.setting.eml-filter-based' ).prevAll( '.setting' ).first().after( media.template( 'eml-pro-gallery-additional-params' )(this.options) );
            }
            else {
                $el.find( '.setting' ).last().after( media.template( 'eml-pro-gallery-additional-params' )(this.options) );
            }
            ...
            if ( 'rand' === orderby || 'menuOrder' === orderby ) {
                $el.find( '.order' ).parent( 'label' ).hide();
            }
    ...
    _.extend( media.view.Settings.Playlist.prototype, {
        ...
        render: function() {
            ...
            if ( $el.find( '.setting.eml-filter-based' ).length ) {
                $el.find( '.setting.eml-filter-based' ).prevAll( '.setting' ).first().after( media.template( 'eml-pro-playlist-additional-params' )(this.options) );
            }
            else {
                $el.find( '.setting' ).last().after( media.template( 'eml-pro-playlist-additional-params' )(this.options) );
            }
